    ...
    for (let i = 0; i < allExcludes.length; i++) {
      ...
      for (let j = 0; j < ancestorsArray.length; j++) {
        ...
        if (exclude.source == ancestor) {
          MxUtils.alert(
            "CST09. Can't require this feature as it's been excluded by an ancestor."
          );
          that.graph.removeCells([edge]);
        }
      }
...
export const messageInCst10 = (that, edge) => {
  MxUtils.alert(
    "CST10. Can't require this feature as it's been excluded by an ancestor in an alternative relationship."
  );
  that.graph.removeCells([edge]);
}
...
export const applyRules = (that, evt) => {
  ...
  // Emitir el evento de conexión de nodos
  if (that.relationType && source && target) {
